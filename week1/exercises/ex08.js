...
const repeatNumbers = function (data) {
  ...
  const result = data.map((arr) => {
    // inner array [numberToRepeat, repetitions]
    const numberToRepeat = arr[0];
    const repetitions = arr[1];
    let innerResult = "";
    // loop to increment the number into result
    for (let i = 0; i < repetitions; i++) {
      innerResult += numberToRepeat;
    }
    return innerResult;
  });
  // return result
